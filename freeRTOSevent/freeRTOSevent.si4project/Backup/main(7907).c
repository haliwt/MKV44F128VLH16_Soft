  ...
  while(1)
    {
         ...
         
        {
            ...
            {
             /* 发送失败，即使等待了10个时钟节拍 */
	  			 printf("vTaskMsgRro，即使等待了10个时钟节拍\r\n");
            }
            else
            {
             /* 发送成功 */
				printf("TaskMsgRro，向xQueue1发送数据成功\r\n");							
            }
            /*************************xQueue2************************************/
            /* ?ò???￠?óáD·￠êy?Y￡?è?1????￠?óáD?úá?￡?μè′y10??ê±?ó?ú?? */
            ucBLDC=0x0a;
            ...
            {
             /* 发送失败，即使等待了10个时钟节拍 */
	   	printf("vTaskDIR，即使等待了20个时钟节拍\r\n");
            }
            else
            {
             /* 发送成功 */
		 
                  printf("DIR send ，向xQueue2发送数据成功\r\n");	
            }
            ...
            {
             /* 发送失败，即使等待了10个时钟节拍 */
	   	printf("vTaskBLDC，即使等待了10个时钟节拍\r\n");
            }
            else
            {
             /* 发送成功 */
	       printf("BLDC send ，向xQueue3发送数据成功\r\n");						
            }
#endif 
            ucBLDC++;
            ...
            {
             /* 发送失败，即使等待了10个时钟节拍 */
	   	printf("vTaskLED，即使等待了10个时钟节拍\r\n");
            }
            else
            {
             /* 发送成功 */
	       printf("vTaskMsgLED ，向xQueue4发送数据成功\r\n");						
            }
         }
...
static void vTaskLED(void *pvParameters)
{
  ...
  BaseType_t xResult;
    ...
    while(1)
    {
      ...
      printf("vTaskLED-2 \r\n");
              xResult = xQueueReceive(xQueue4,                   /* ???￠?óáD??±ú */
		...
		//vTaskDelay(200);//taskYIELD();              //放弃时间片，把CPU让给同优先级的其它任务
    }
...
static void vTaskDIR(void *pvParameters)
{
    ...
    {           printf("vTaskDIR-3 \r\n");
		...
		{
			/* 成功接收，并通过串口将数据打印出来 */
                      printf("接收到消息队列数据vTaskDIR = %d\r\n",ucQueueMsgValue );
			//printf("接收到消息队列数据ptMsg->ucMessageID = %d\r\n", ptMsg->ucMessageID);
			//printf("接收到消息队列数据ptMsg->ulData[3] = %d\r\n", ptMsg->ulData[3]);
			//printf("接收到消息队列数据ptMsg->usData[3] = %d\r\n", ptMsg->usData[3]);
			Dir = Dir;
		}
		else
		{
...
static void vTaskBLDC(void *pvParameters)
{
    ...
    {           printf("vTaskDIR-4 \r\n");
		...
		vTaskDelay(300);//taskYIELD();              //放弃时间片，把CPU让给同优先级的其它任务
    } 
